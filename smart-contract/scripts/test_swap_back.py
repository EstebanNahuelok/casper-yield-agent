#!/usr/bin/env python3
"""
test_swap_back.py — Ejecuta execute_swap(sCSPR -> CSPR) para verificar el swap inverso.

Requiere que:
  1. El pool y vault estén actualizados con upgrade_contracts.mjs
  2. El vault tenga sCSPR acumulado (haber corrido test_swap.py antes)

Usage (desde repo root, con venv activo):
  python smart-contract/scripts/test_swap_back.py
"""
...
# sCSPR virtual units to swap back (in motes: 1 sCSPR = 1e9 motes)
SWAP_BACK_SCSPR = 4.0
...
async def run_test_swap_back(vault_pkg: str, scspr_amount: float, api_key: str, secret_key_b64: str):
    import pycspr
    ...
    amount_in_motes = int(scspr_amount * 1_000_000_000)

    arguments = [
        DeployArgument("token_in",   CLV_String("sCSPR")),
        DeployArgument("token_out",  CLV_String("CSPR")),
        DeployArgument("amount_in",  CLV_U512(amount_in_motes)),
        DeployArgument("amount_out", CLV_U512(0)),
    ]
    ...
    print(f"\nCalling execute_swap({scspr_amount} sCSPR -> CSPR) on vault {vault_pkg[:16]}...")
    tx_hash = await client.account_put_deploy(deploy)
    print(f"\n[OK] execute_swap (reverse) submitted!")
...
def main():
    ...
    print(f"SWAP_BACK_SCSPR     : {SWAP_BACK_SCSPR}")
    print()

    if sys.platform == "win32":
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    asyncio.run(run_test_swap_back(vault_pkg, SWAP_BACK_SCSPR, api_key, secret_key))
