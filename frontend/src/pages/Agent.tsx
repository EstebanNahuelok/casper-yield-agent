...
function ParamSlider({
  ...
  return (
    <div className={`p-4 rounded-xl border border-zinc-900 bg-zinc-900/30 ${comingSoon ? "opacity-60" : ""}`}>
      ...
      <input
        ...
        value={value}
        disabled={comingSoon}
        onChange={(e) => !comingSoon && onChange(parseFloat(e.target.value))}
        className={`w-full accent-red-500 ${comingSoon ? "cursor-not-allowed" : ""}`}
      />
    </div>
  );
}
